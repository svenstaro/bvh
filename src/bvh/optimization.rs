...
impl BVH {
    ...
    /// children.
    fn update<Shape: BHShape>(&mut self,
                              node_index: usize,
                              shapes: &[Shape])
    ...
    fn fix_aabbs<Shape: BHShape>(&mut self,
                                 node_index: usize,
                                 shapes: &[Shape])
                                 -> Option<OptimizationIndex> {
    ...
    fn rotate<Shape: BHShape>(&mut self,
                              node_a_index: usize,
                              node_b_index: usize,
                              shapes: &[Shape]) {
        ...
        // Perform the switch
        self.connect_nodes(node_a_index,
                           node_b_parent_index,
                           node_b_is_left_child,
                           shapes);
        self.connect_nodes(node_b_index,
                           node_a_parent_index,
    ...
    fn connect_nodes<Shape: BHShape>(&mut self,
                                     child_index: usize,
                                     parent_index: usize,
                                     left_child: bool,
                                     shapes: &[Shape]) {
...
pub mod tests {
    ...
    use testbase::{build_some_bh, create_n_cubes, load_sponza_scene, intersect_bh, UnitBox,
                   randomly_move_triangles, randomly_transform_scene, default_bounds};
    use bounding_hierarchy::BHShape;
    use std::f32;
    ...
    fn test_optimize_simple_update() {
        ...
        // Assert that SAH joined shapes #0 and #1.
        {
            ...
            match (&bvh.nodes[left.bh_node_index()], &bvh.nodes[moving.bh_node_index()]) {
                (&BVHNode::Leaf { parent_index: left_parent_index, .. },
                 &BVHNode::Leaf { parent_index: moving_parent_index, .. }) => {
        ...
        // Assert that now SAH joined shapes #1 and #2.
        {
            ...
            match (&bvh.nodes[right.bh_node_index()], &bvh.nodes[moving.bh_node_index()]) {
                (&BVHNode::Leaf { parent_index: right_parent_index, .. },
                 &BVHNode::Leaf { parent_index: moving_parent_index, .. }) => {
    ...
    fn bench_intersect_after_optimize(b: &mut ::test::Bencher) {
        ...
        for _ in 0..50 {
            let updated = randomly_move_triangles(&mut triangles, 120_000, &bounds, &mut seed);
            bvh.optimize(&updated, &triangles);
        }
    ...
    fn bench_intersect_with_rebuild(b: &mut ::test::Bencher) {
        ...
        for _ in 0..50 {
            randomly_move_triangles(&mut triangles, 120_000, &bounds, &mut seed);
        }
    ...
    #[bench]
    /// Benchmark intersecting a `BVH` after randomly moving the triangles and optimizing.
    fn bench_intersect_sponza_after_optimize(b: &mut ::test::Bencher) {
        // Compute bounds // TODO
        ...
        for _ in 0..10 {
            let updated = randomly_transform_scene(&mut triangles, 120_000, &bounds, &mut seed);
            bvh.optimize(&updated, &triangles);
        }
    ...
    /// Used to compare optimizing with rebuilding. For reference see
    /// `bench_intersect_after_optimize`.
    fn bench_intersect_sponza_with_rebuild(b: &mut ::test::Bencher) {
        let (mut triangles, bounds) = load_sponza_scene();
        let mut seed = 0;

        for _ in 0..10 {
            randomly_transform_scene(&mut triangles, 120_000, &bounds, &mut seed);
        }
        let bvh = BVH::build(&mut triangles);
