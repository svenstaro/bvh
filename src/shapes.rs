...
impl Capsule {
    pub fn new(start: Point3, end: Point3, radius: f64) -> Capsule
    {
        let line = end - start;
...
impl IntersectionTest for OBB {
    ...
    fn intersects_aabb(&self, aabb: &AABB) -> bool {
        ...
        let mat = DMat4::from_rotation_translation(self.orientation, translation);

        let vec_1 = Vector3::new(translation.x.abs(), translation.y.abs(), translation.z.abs());
        let right = right(mat);
        let up = up(mat);
        ...
        if translation.dot(right.abs()) > half_a.x * right.x.abs() + half_a.y * right.y.abs() + half_a.z * right.z.abs() + half_b.x
        {
            return false;
        }
        if translation.dot(up.abs()) > half_a.x * up.x.abs() + half_a.y * up.y.abs() + half_a.z * up.z.abs() + half_b.y
        {
            return false;
        }
        if translation.dot(backward.abs()) > half_a.x * backward.x.abs() + half_a.y * backward.y.abs() + half_a.z * backward.z.abs() + half_b.z
        {
            return false;
        }
        let mut vec_5 = Vector3::new(0.0, -right.z, right.y);
        if translation.dot(vec_5.abs()) > half_a.y * vec_5.y.abs() + half_a.z * vec_5.z.abs() + vec_5.dot(vec_3.abs()) + vec_5.dot(vec_4.abs())
        {
            return false;
        }
        vec_5 = Vector3::new(0.0, -up.z, up.y);
        if translation.dot(vec_5.abs()) > half_a.y * vec_5.y.abs() + half_a.z * vec_5.z.abs() + vec_5.dot(vec_4.abs()) + vec_5.dot(vec_2.abs())
        {
            return false;
        }
        vec_5 = Vector3::new(0.0, -backward.z, backward.y);
        if translation.dot(vec_5.abs()) > half_a.y * vec_5.y.abs() + half_a.z * vec_5.z.abs() + vec_5.dot(vec_2.abs()) + vec_5.dot(vec_3.abs())
        {
            return false;
        }
        vec_5 = Vector3::new(right.z, 0.0, -right.x);
        if translation.dot(vec_5.abs()) > half_a.z * vec_5.z.abs() + half_a.x * vec_5.x.abs() + vec_5.dot(vec_3.abs()) + vec_5.dot(vec_4.abs())
        {
            return false;
        }
        vec_5 = Vector3::new(up.z, 0.0, -up.x);
        if translation.dot(vec_5.abs()) > half_a.z * vec_5.z.abs() + half_a.x * vec_5.x.abs() + vec_5.dot(vec_4.abs()) + vec_5.dot(vec_2.abs())
        {
            return false;
        }
        vec_5 = Vector3::new(backward.z, 0.0, -backward.x);
        if translation.dot(vec_5.abs()) > half_a.z * vec_5.z.abs() + half_a.x * vec_5.x.abs() + vec_5.dot(vec_2.abs()) + vec_5.dot(vec_3.abs())
        {
            return false;
        }
        vec_5 = Vector3::new(-right.y, right.x, 0.0);
        if translation.dot(vec_5.abs()) > half_a.x * vec_5.x.abs() + half_a.y * vec_5.y.abs() + vec_5.dot(vec_3.abs()) + vec_5.dot(vec_4.abs())
        {
            return false;
        }
        vec_5 = Vector3::new(-up.y, up.x, 0.0);
        if translation.dot(vec_5.abs()) > half_a.x * vec_5.x.abs() + half_a.y * vec_5.y.abs() + vec_5.dot(vec_4.abs()) + vec_5.dot(vec_2.abs())
        {
            return false;
        }
        vec_5 = Vector3::new(-backward.y, backward.x, 0.0);
        if translation.dot(vec_5.abs()) > half_a.x * vec_5.x.abs() + half_a.y * vec_5.y.abs() + vec_5.dot(vec_2.abs()) + vec_5.dot(vec_3.abs())
        {
            return false;
        }
        // Intersection
...
fn right(matrix: DMat4) -> Vector3
{
    matrix.row(0).truncate()
}

fn up(matrix: DMat4) -> Vector3
{
    matrix.row(1).truncate()
}

fn back(matrix: DMat4) -> Vector3
{
    matrix.row(2).truncate()
}

fn translation(matrix: DMat4) -> Vector3
{
    matrix.row(3).truncate()
}

pub fn nearest_point_on_line(p1: &Point3, dir: &Vector3, len: f64, pnt: &Point3) -> Point3 {
    let v = *pnt - *p1;
    let d = v.dot(*dir);
...
mod tests {
    ...
    fn basic_obb() {
        ...
        let ori = DQuat::from_axis_angle(Vector3::new(1.0, 0.0, 0.0), 0.785398);
